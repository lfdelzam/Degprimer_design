...
parser.add_argument(
    '-o', dest='o', help='output file, list of selected primers', required=True)
parser.add_argument(
    '-d', dest='d', help='output directory, default (current directory)', default=os.getcwd())
parser.add_argument(
    '-c', dest='c', help='coverage threshold, fraction, default 0.98', default=0.98)
parser.add_argument(
    '-g', dest='g', help='gc content threshold, in percentage, default 30', default=30)
#parser.add_argument(
    ...
    for it in range(0, len(primer)):
        ...
        while pr < total_deg:  # for all the primers
            if deg in degen.keys():
                ...
                # number of times the same nucleotide should be used for if window change
                window = total_deg/Ntc
                for ch in range(0, Nn):  # for each nucleotide in degenerated nucleotide
                    counter = 0
...
outfile=os.path.join(dir, args.o)
