        ...
        for line in fin:
            ...
            if line.startswith("Primer_"):
                ...
                if prim not in bad_primers:
                    if not prim in PCR_problem: #initializing primer count
                    ...
                    if prim in primers_info:
                        ...
                        primers_info[prim]={"Length": int(line.split()[2]), "GC": [float(line.split()[3])], "TM": [float(line.split()[4])],
                                               "pos": pos}
...
problem="Dimer "
problem_list="Dimer List"
bad_primers = bad_primer_list(args.i, problem, problem_list)
PCR_problem, primers_info = p_to_p_dimers(args.i, problem, problem_list, bad_primers)
    ...
    for i in range(0,len(list_of_primers)):
        ...
        for o in range(i+1,len(list_of_primers)):
            ...
            if primer_pair not in no_pair:
                ...
                if size >= int(args.m) and size <= int(args.M) and delta_TMmax <= 5 and delta_TMmin <= 5:
                    print("{} {} {} {} {}".format(args.s+"_"+str(counter), primer+"_at_"+primers_info[primer]["pos"], Sequences[primer], primer_pair+"_at_"+primers_info[primer_pair]["pos"], revcomp(Sequences[primer_pair]) ), file=fout )
                    counter += 1
        print("{}\t{}\t{}\t{}\t{}\t({}-{})\t({}-{})".format(primer+"_at_"+pos_ini, Sequences[primer], revcomp(Sequences[primer_pair]), degenerecy(Sequences[primer]), primers_info[primer]["Length"], min(primers_info[primer]["GC"]),  max(primers_info[primer]["GC"]), TM_min, TM_max), file=fo)
